...
//!
//! Adapted from Candle's JinaBERT implementation for the QK post-norm
//! architecture used by `jinaai/jina-embeddings-v2-base-code`.

use candle_core::{D, DType, Device, IndexOp, Result, Tensor};
use candle_nn::{Embedding, Linear, embedding, linear, linear_no_bias};
...
impl BertSelfAttention {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let layer_norm_q = layer_norm(hidden_size, cfg.layer_norm_eps, vb.pp("layer_norm_q"))?;
        let layer_norm_k = layer_norm(hidden_size, cfg.layer_norm_eps, vb.pp("layer_norm_k"))?;
        Ok(Self {
            query,
            key,
            value,
